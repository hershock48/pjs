 ...
 *
 * AN EMAIL PROBLEM MUST NEVER FAIL AN ORDER. Every caller awaits this but
 * swallows rejections. Awaited, not fire-and-forget, because an un-awaited
 * send dies on serverless: Vercel freezes the lambda the moment the response
 * returns, so the send silently never runs. Swallowed, because a guest whose
 * order is in the kitchen and whose receipt bounced has an order; a guest
 * whose order was rejected because mail was down has nothing.
 *
