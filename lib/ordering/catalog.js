...
function toOrderable(doc, { includeHidden, at }) {
  return doc
    .map((s) => ({
      name: s.name,
      ...
      items: s.items
        ...
        // `at` absent or null on an item means both counters. This filter is the
        // only place location narrowing happens, so the order page and the order
        // route cannot end up disagreeing about where a pizza can be bought.
        .filter((i) => !at || !i.at || i.at.includes(at))
        .map((i) => ({
          id: i.id,
          section: s.name,
          name: i.name,
 ...
 *
 * `at` narrows to one counter's board. The ORDER PAGE deliberately calls this
 * WITHOUT it and ships the whole board with an `at` on every item, so switching
 * counters is instant. The ORDER ROUTE calls it WITH the ordered location, so a
 * Marshall-only item on a Battle Creek ticket is refused at the point it would
 * otherwise be priced. Same field, same function, one of them enforcing.
 */
export async function guestMenu(store, at = null) {
  const doc = await loadMenuDoc(store);
  const sections = toOrderable(doc, { includeHidden: false, at });
...
export function validateMenuDoc(doc) {
  ...
  for (const s of doc) {
    ...
    for (const i of s.items) {
      ...
      // Which counters serve it. Absent or null means both. An empty array is
      // rejected rather than treated as "both", because it reads as "nowhere"
      // and would silently take the item off every board.
      if (i.at != null) {
        if (!Array.isArray(i.at) || i.at.length === 0)
          return `Locations on ${i.name} must be a non-empty list, or left out for both counters.`;
        if (i.at.some((s) => typeof s !== "string" || !s.trim()))
          return `Malformed location on ${i.name}.`;
      }
